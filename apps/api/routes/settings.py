...
from estimator.pricing import DEFAULT_MODELS, get_model, list_models
...
async def put_settings(payload: SettingsUpdate, user: dict = Depends(get_current_user)):
    ...
    if payload.default_model is not None:
        try:
            get_model(payload.default_model, provider)
        except KeyError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        fields["default_model" if provider is LLMProvider.GROQ else "openrouter_default_model"] = payload.default_model
